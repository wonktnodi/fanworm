...
func (t *Timer) add(td *TimerData) {
	...
	if td.index == 0 {
		...
		t.signal.Reset(d)
		if Debug {
			log.Debugf("timer: add reset delay %d ms", int64(d)/int64(time.Millisecond))
		}
	}
	if Debug {
...
func (t *Timer) expire() {
	...
	t.signal.Reset(d)
	if Debug {
		log.Debugf("timer: expire reset delay %d ms", int64(d)/int64(time.Millisecond))
	}
	t.lock.Unlock()
	return
}
